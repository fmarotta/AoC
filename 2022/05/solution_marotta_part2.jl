...
# there are. Also, let's pretend that we can't use arrays.
...
function append_in_order!(stack::Stack{T}, substack::Vector{T}) where T
...
function pop_in_order!(stack::Stack{T}, n::Int) where T
    substack = Vector{T}(undef, n)
    while n > 0
        node = stack.top
        stack.top = node.prev
        substack[n] = node.payload
        n -= 1
    end
    return substack
end
...
while !eof(io)
    ...
    println(to)
    println()
    substack = pop_in_order!(stacks[from], how_many)
    append_in_order!(stacks[to], substack)
end
