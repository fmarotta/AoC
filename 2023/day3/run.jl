...
function gobble_number(m, start_r, start_c)
    s = 0
    rightmost = start_c
            ...
            for dr in (-1, 0, 1)
                for dc in (-1, 0, 1)
                    ...
                    if isdigit(schematic[r + dr, c + dc])
                        n, start_r, start_c = gobble_number(schematic, r + dr, c + dc)
...
function find_gear_ratios(schematic, stars)
    ...
    for v in values(stars)
        ...
        n_1, _, _ = gobble_number(schematic, v[1][1], v[1][2])
        n_2, _, _ = gobble_number(schematic, v[2][1], v[2][2])
        tot += n_1 * n_2
    end
    return tot
end

schematic = read_puzzle("input")
