...
function check_part(part, blocks, memo = Dict())
    if (part, join(blocks)) in keys(memo)
        return memo[(part, join(blocks))]
    end
    if count('#', part) > sum(blocks)
        memo[(part, join(blocks))] = 0
        return 0
    end
    if length(part) == 0
        if sum(blocks) == 0
            memo[(part, join(blocks))] = 1
            return 1
        else
            memo[(part, join(blocks))] = 0
            return 0
    ...
    elseif startswith(part, '?')
        ...
        memo[(part[2:end], join(blocks))] = p1
        bb = copy(blocks)
        while (startswith(part, '#') || startswith(part, '?')) && bb[1] > 0
            part = part[2:end]
            bb[1] -= 1
